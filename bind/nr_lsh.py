...
def multiprobe(num_partitions, bits, dim, num_buckets, dtype=float32):
    if any([dtype is f for f in _valid_float32]):
        return nr_binding.MultiProbeFloat(num_partitions, bits, dim, num_buckets)
    elif any([dtype is f for f in _valid_float64]):
        return nr_binding.MultiProbeDouble(num_partitions, bits, dim, num_buckets)
    else:
        raise ValueError('multiprobe', 'invalid dtype', 'dtype='+str(dtype))

multiprobe(1, 1, 1, 1, 'float32')
